...
核心目标：在高峰期抢指定日期、指定时间段的场地。尽量按目标场地预订，不一定一次请求都要完全满足，有多少订多少。绝对禁止有场地但不提交请求。每次拉矩阵，只要包含符合需求的，必须至少拿1块。

## 注意事项

用户需求目标：满足「指定块数 + 指定时段」即可，不要求必须固定场号、连续号码。
...
以下为根据多次12 点抢场实战复盘归纳的**规则、经验与教训**，供配置与排错时对照。

1. **提交订单的条件**  
   必须在提交订单前有一次查询矩阵（`get_matrix` / `getPlaceInfoByShortName`）拉活；约 10 秒窗口内 POST 较易成功，否则会常见「数据错误请重试」。  
   一次成功的 POST 后可继续 POST；**相邻两次 POST 之间须间隔足够长**（实践中常需 **>5s**），否则易触发 too fast；由 `config.json` 的 **`delivery_min_post_interval_seconds`**（系统配置 → 任务执行参数）统一控制，程序用 **sleep** 保证间隔，**不必**在拉矩阵后再单独多等 5 秒。  
   出现业务/数据类错误后应**再次拉矩阵**再重试；重试前的等待仍由「距上一次 POST 已超过 `delivery_min_post_interval_seconds`」保证，与上一条一致。
...
3. **首矩阵要尽早拿到，拿到后立即发订单**  
   黄金窗口只有几分钟，拿到矩阵是最重要的，期间网络的延时比较严重，程序应该设计足够的超时时间，避免自己过早放弃。

4. **“接口已 success”基本上99%成功**  
   只有接口明确返回错误意义：操作过快是大概率是两次post的间隔过短，数据错误是缺少前置的get_matrix，日期错误大多是已经被预订了。矩阵格子状态判断不要用mine覆盖（include_mine_overlay）判断是否被我预订，直接读取原始数据状态即可。
